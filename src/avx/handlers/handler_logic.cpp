...
merror_t handle_v_bitwise(codegen_t &cdg)
{
  ...
  {
    case NN_vpand: case NN_vandps: case NN_vandpd: opname = "and"; break;
    case NN_vpor:  case NN_vorps:  case NN_vorpd:  opname = "or";  break;
    case NN_vpxor: case NN_vxorps: case NN_vxorpd: opname = "xor"; break;
...
merror_t handle_vshufps(codegen_t &cdg)
{
  ...
  qstring iname; iname.cat_sprnt("_mm%s_shuffle_ps", size==YMM_SIZE ? "256" : "");
  AVXIntrinsic icall(&cdg, iname.c_str());
...
merror_t handle_vbroadcast_ss_sd(codegen_t &cdg)
{
  ...
  AVXIntrinsic icall(&cdg, (size==YMM_SIZE) ? (is_double ? "_mm256_set1_pd" : "_mm256_set1_ps")
...
merror_t handle_vcmp_ps_pd(codegen_t &cdg)
{
  ...
  qstring iname; iname.cat_sprnt("_mm%s_cmp_%s", size==YMM_SIZE ? "256" : "", suf);
  AVXIntrinsic icall(&cdg, iname.c_str());
  tinfo_t vt = get_type_robust(size, false, is_double);
...
merror_t handle_vblendv_ps_pd(codegen_t &cdg)
{
  ...
  qstring iname; iname.cat_sprnt("_mm%s_blendv_%s", size==YMM_SIZE ? "256" : "", is_double ? "pd" : "ps");
  AVXIntrinsic icall(&cdg, iname.c_str());
  tinfo_t vt = get_type_robust(size, false, is_double);
...
merror_t handle_vmaskmov_ps_pd(codegen_t &cdg)
{
  ...
  int size = (is_xmm_reg(cdg.insn.Op1) || (is_mem_op(cdg.insn.Op1) && is_xmm_reg(cdg.insn.Op2))) ? XMM_SIZE : YMM_SIZE;
  tinfo_t vt = get_type_robust(size, false, is_double);
  ...
  if (is_avx_reg(cdg.insn.Op1))
  {
    ...
    const char *setz = (size==YMM_SIZE) ? (is_double ? "_mm256_setzero_pd" : "_mm256_setzero_ps")
    ...
    const char *blend = (size==YMM_SIZE) ? (is_double ? "_mm256_blendv_pd" : "_mm256_blendv_ps")
