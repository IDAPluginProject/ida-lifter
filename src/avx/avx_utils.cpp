...
{
  qstring n; n.cat_sprnt(fmt, op_size == YMM_SIZE ? "256" : "");
  return n;
}
...
struct InsnMapping { uint16 avx_itype; uint16 sse_itype; };
...
bool is_misc_insn(uint16 it) { return it==NN_vsqrtss||it==NN_vsqrtps||it==NN_vshufps||it==NN_vzeroupper; }
